...
module.exports = function (req, res) {
  ...
  Event.findById(eventId, function(err, eventData) {
    if(err) {
      ...
      if(eventData.owner === req.username) {
        deleteCommitters(eventId, req);
      } else {
        res.status(403).json({msg: 'You are not authorized to delete this file'});
      }
    }
  })
  ...
  function deleteCommitters(eventId, req) {
    Committer.remove({eventToAttend: eventId}, function(err, data) {
      if(err) {
        res.status(500).json({msg: 'Server error: cannot remove Committers'});
      } else {
  ...
  function deleteEvent(eventId, req) {
    ...
    Event.findByIdAndRemove(eventId, function(err, data) {
      if(err) {
        ...
        res.json({msg:'Event and Committers successfully deleted', data: data});
      }
    })
  }
}
