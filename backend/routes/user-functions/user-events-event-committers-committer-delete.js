...
module.exports = function (req, res) {
  ...
  //verify if token holder owns the event that this committer is attending that event


  //find event that committer is attending
  Committer.findById(committerId, function(err, committer) {
    if (err) {
      res.status(500).json({msg: 'Server error: cannot get committer'});
    } else {
      verifyOwnerAndDelete(committer.eventToAttend, req);
    }
  });
    ...
    Event.findById(eventId, function(err, eventData) {
      if(err) {
        ...
        if(eventData.owner === req.username) {
          updateEventAndDeleteCommitter(committerId, eventId, req)
        } else {
  ...
  function updateEventAndDeleteCommitter(committerId, eventId, req) {
    ...
    Committer.findByIdAndRemove(committerId, function(err, committer) {
      if (err) {
        res.status(500).json({msg: 'Cannot delete this committer'})
      } else {
        updateEvent(committer, eventId, req);
      }
    });
  }
